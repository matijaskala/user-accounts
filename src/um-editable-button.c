                                 ...
                                 gboolean          editable)
{
        ...
        if (priv->editable != editable) {
                ...
                if (editable) {
                        gtk_widget_set_visible(GTK_WIDGET(priv->button), TRUE);
                        gtk_widget_set_visible(GTK_WIDGET(priv->label), FALSE);
                }
                else {
                        gtk_widget_set_visible(GTK_WIDGET(priv->button), FALSE);
...
update_button_padding (UmEditableButton *button, gpointer user_data)
{
        ...
        context = gtk_widget_get_style (GTK_WIDGET (priv->button));
        state = gtk_style_get_state (context);

        gtk_style_get_padding (context, state, &padding);
        gtk_style_get_border (context, state, &border);
...
um_editable_button_init (UmEditableButton *button)
{
        ...
        priv->stack = GTK_BOX (gtk_vbox_new (0, 0));
