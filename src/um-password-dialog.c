...
um_password_dialog_new (void)
{
        ...
        widget = gtk_dialog_add_button (GTK_DIALOG (um->dialog), _("_Cancel"), GTK_RESPONSE_CANCEL);
        gtk_button_set_use_underline (GTK_BUTTON (widget), TRUE);
        g_signal_connect (widget, "clicked",
                          G_CALLBACK (cancel_password_dialog), um);

        widget = gtk_dialog_add_button (GTK_DIALOG (um->dialog), _("Ch_ange"), GTK_RESPONSE_OK);
        gtk_button_set_use_underline (GTK_BUTTON (widget), TRUE);
        g_signal_connect (widget, "clicked",
                          G_CALLBACK (accept_password_dialog), um);
        gtk_widget_grab_default (widget);
        um->ok_button = widget;
        gtk_dialog_set_default_response (GTK_DIALOG (um->dialog), GTK_RESPONSE_OK);
