...
set_fingerprint_label (GtkWidget *editable_button)
{
        ...
        if (fingers == NULL || g_variant_iter_n_children (fingers) == 0) {
                is_disable = FALSE;
                gtk_button_set_label (GTK_BUTTON (editable_button), _("Disabled"));
        } else {
                is_disable = TRUE;
                gtk_button_set_label (GTK_BUTTON (editable_button), _("Enabled"));
        }
...
enroll_result (EnrollData *data, const char *result, gboolean done)
{
        ...
        if (g_str_equal (result, "enroll-completed") || g_str_equal (result, "enroll-stage-passed")) {
                ...
                path = g_strdup_printf (DATADIR "/icons/print_ok.png");
...
assistant_prepare (GtkAssistant *ass, GtkWidget *page, EnrollData *data)
{
        ...
        if (g_str_equal (name, "enroll")) {
                ...
                {
                        path = g_strdup_printf (DATADIR "/icons/%s.png", data->finger);
                }
                     ...
                     ActUser   *user)
{
        ...
        if (!gtk_builder_add_from_file (dialog,
                                            DATADIR "/account-fingerprint.ui",
                                            &error)) {
